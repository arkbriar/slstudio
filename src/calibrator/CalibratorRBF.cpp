...
CalibratorRBF::CalibratorRBF(unsigned int _screenCols, unsigned int _screenRows)
    ...
    for (unsigned int i = 0; i < N; i++) patterns.push_back(encoder->getEncodingPattern(i));
}
...
CalibrationData CalibratorRBF::calibrate() {
    ...
    for (unsigned int i = 0; i < nFrameSeq; i++) {
        ...
        bool success =
            cv::findChessboardCorners(shading[i], patternSize, qci, cv::CALIB_CB_ADAPTIVE_THRESH);
        if (!success)
            std::cout << "Calibrator: could not extract chess board corners on frame seqence " << i
                      << std::endl
                      << std::flush;
        else
    ...
    double cam_error = cv::calibrateCamera(
        ...
        cv::CALIB_FIX_ASPECT_RATIO + cv::CALIB_FIX_PRINCIPAL_POINT + cv::CALIB_FIX_K2 +
    ...
    //        std::cout << "Seq error " << i+1 << " cam:" << cam_error_per_view[i] << " proj:" <<
    //        proj_error_per_view[i] << std::endl;
    //    }
    ...
    //        fs << QString("proj_error_per_view_%1").arg(i).toStdString() <<
    //        proj_error_per_view[i];
    //    }
    //    fs.release();
