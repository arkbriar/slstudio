...
void RBFInterpolator::setDataPoints(const std::vector<cv::Point2f> x,
                                    const std::vector<cv::Point2f> f) {
    unsigned int dim = 2;
    ...
    A(cv::Range(0, N), cv::Range(0, N)) =
        Psi + cv::Mat::eye(N, N, CV_32F) * regularizationK;  // add regularization
    P.copyTo(A(cv::Range(0, N),
               cv::Range(N, N + dim + 1)));  // needs copyTo because P is not a temporary
    P = P.t();
...
cv::Point2f RBFInterpolator::interpolate(const std::vector<cv::Point2f> x,
                                         const cv::Point2f xStar) {
    unsigned int N = x.size();
