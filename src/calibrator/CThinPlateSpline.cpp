...
void CThinPlateSpline::setCorrespondences(const std::vector<Point2f>& pS,
                                          const std::vector<Point2f>& pD) {
    pSrc = pS;
    pDst = pD;
...
void CThinPlateSpline::computeSplineCoeffs(std::vector<Point2f>& iPIn, std::vector<Point2f>& iiPIn,
                                           float lambda, const TPS_INTERPOLATION tpsInter) {
...
void CThinPlateSpline::warpImage(const Mat& src, Mat& dst, float lambda, const int interpolation,
...
void CThinPlateSpline::computeMaps(const Size& dstSize, Mat_<float>& mx, Mat_<float>& my,
                                   const TPS_INTERPOLATION tpsInter) {
