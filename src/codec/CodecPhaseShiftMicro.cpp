...
static const float frequencies[] = {75.02, 70.00, 71.32, 72.47, 73.72,
                                    76.23, 77.35, 78.40, 79.22, 80.00};

// Encoder
EncoderPhaseShiftMicro::EncoderPhaseShiftMicro(unsigned int _screenCols, unsigned int _screenRows,
                                               CodecDir _dir)
...
cv::Mat EncoderPhaseShiftMicro::getEncodingPattern(unsigned int depth) { return patterns[depth]; }

// Decoder
DecoderPhaseShiftMicro::DecoderPhaseShiftMicro(unsigned int _screenCols, unsigned int _screenRows,
                                               CodecDir _dir)
...
void DecoderPhaseShiftMicro::setFrame(unsigned int depth, cv::Mat frame) { frames[depth] = frame; }

void DecoderPhaseShiftMicro::decodeFrames(cv::Mat &up, cv::Mat &vp, cv::Mat &mask,
                                          cv::Mat &shading) {
    const float pi = M_PI;
    ...
    for (int i = 0; i < F - 1; i++) Mmicro.at<float>(3 + i, 3 + i) = (i % 2) * 2 - 1;
