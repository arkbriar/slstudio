...
static inline int powi(int num, unsigned int exponent) {
    ...
    float res = num;
    for (unsigned int i = 0; i < exponent - 1; i++) res *= num;

    return res;
}

// Encoder
EncoderGrayCode::EncoderGrayCode(unsigned int _screenCols, unsigned int _screenRows, CodecDir _dir)
    : Encoder(_screenCols, _screenRows, _dir) {
    N = 2;

    // Set total pattern number
    if (dir & CodecDirHorizontal) this->N += Nhorz;

    if (dir & CodecDirVertical) this->N += Nvert;
...
cv::Mat EncoderGrayCode::getEncodingPattern(unsigned int depth) { return patterns[depth]; }

// Decoder
DecoderGrayCode::DecoderGrayCode(unsigned int _screenCols, unsigned int _screenRows, CodecDir _dir)
...
void DecoderGrayCode::setFrame(unsigned int depth, const cv::Mat frame) { frames[depth] = frame; }
