...
// Encoder
EncoderPhaseShift2p1::EncoderPhaseShift2p1(unsigned int _screenCols, unsigned int _screenRows,
                                           CodecDir _dir)
...
cv::Mat EncoderPhaseShift2p1::getEncodingPattern(unsigned int depth) { return patterns[depth]; }

// Decoder
DecoderPhaseShift2p1::DecoderPhaseShift2p1(unsigned int _screenCols, unsigned int _screenRows,
                                           CodecDir _dir)
...
void DecoderPhaseShift2p1::setFrame(unsigned int depth, cv::Mat frame) { frames[depth] = frame; }
...
void DecoderPhaseShift2p1::decodeFrames(cv::Mat &up, cv::Mat &vp, cv::Mat &mask, cv::Mat &shading) {
    ...
    if (lastShading->empty()) *lastShading = I3;
...
DecoderPhaseShift2p1::~DecoderPhaseShift2p1() {
    ...
    if (!fs.isOpened()) std::cerr << "Could not write shiftHistory.xml" << std::endl;
