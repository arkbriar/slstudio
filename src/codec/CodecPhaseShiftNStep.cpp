...
// Encoder
EncoderPhaseShiftNStep::EncoderPhaseShiftNStep(unsigned int _screenCols, unsigned int _screenRows,
                                               CodecDir _dir)
    : Encoder(_screenCols, _screenRows, _dir) {
    // Set N
    N = nSteps + 3;
    if (dir == CodecDirBoth) N *= 2;
...
cv::Mat EncoderPhaseShiftNStep::getEncodingPattern(unsigned int depth) { return patterns[depth]; }

// Decoder
DecoderPhaseShiftNStep::DecoderPhaseShiftNStep(unsigned int _screenCols, unsigned int _screenRows,
                                               CodecDir _dir)
...
void DecoderPhaseShiftNStep::setFrame(unsigned int depth, cv::Mat frame) { frames[depth] = frame; }

void DecoderPhaseShiftNStep::decodeFrames(cv::Mat &up, cv::Mat &vp, cv::Mat &mask,
                                          cv::Mat &shading) {
    const float pi = M_PI;
