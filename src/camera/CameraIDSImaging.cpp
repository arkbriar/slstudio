...
CameraIDSImaging::CameraIDSImaging(unsigned int camNum, CameraTriggerMode triggerMode)
    : Camera(triggerMode), frameWidth(0), frameHeight(0), frameMemory(NULL), memoryID(0) {
    // Init Camera
    ...
    if (err != IS_SUCCESS) std::cerr << "CameraIDSImaging Error: Failed to set error reporting!";
#endif
    ...
    //        &memoryID[i]); is_AddToSequence(camera, frameMemory[i], memoryID[i]);
    //    }
...
CameraSettings CameraIDSImaging::getCameraSettings() {
    ...
    settings.gain = is_SetHardwareGain(camera, IS_GET_MASTER_GAIN, IS_IGNORE_PARAMETER,
...
void CameraIDSImaging::setCameraSettings(CameraSettings settings) {
    ...
    is_SetHardwareGain(camera, settings.gain, IS_IGNORE_PARAMETER, IS_IGNORE_PARAMETER,
                       IS_IGNORE_PARAMETER);
}
...
// Create the Windows event
    ...
    std::cout << "Gain: "
              << is_SetHardwareGain(camera, IS_GET_MASTER_GAIN, IS_IGNORE_PARAMETER,
                                    IS_IGNORE_PARAMETER, IS_IGNORE_PARAMETER)
              << " %" << std::endl;
...
CameraFrame CameraIDSImaging::getFrame() {
    ...
    if (!capturing) {
        std::cerr << "CameraIDSImaging: capturing. Call startCapture() before lockFrame()"
                  << std::endl;
        return frame;
    }
...
unsigned int CameraIDSImaging::getFrameWidthHeight() { return frameHeight; }
