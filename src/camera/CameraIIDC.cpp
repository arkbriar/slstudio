...
CameraIIDC::CameraIIDC(unsigned int camNum, CameraTriggerMode triggerMode) : Camera(triggerMode) {
    ...
    if (err != DC1394_SUCCESS) cerr << "libdc1394: Could not set video mode or color coding!";
    ...
    if (err != DC1394_SUCCESS) cerr << "libdc1394: Could not set operation mode!";

    err = dc1394_video_set_iso_speed(cam, DC1394_ISO_SPEED_800);
    if (err != DC1394_SUCCESS) cerr << "libdc1394: Could not set iso speed!";
...
// Size of ringbuffer (Note: on OS X 10.9, size must be greater than 1)
    ...
    if (err != DC1394_SUCCESS) cerr << "libdc1394: Could not set up camera!";
...
unsigned int getSetting(dc1394camera_t *cam, dc1394feature_t setting) {
    ...
    if (err != DC1394_SUCCESS) cerr << "Could not get setting!" << endl;
...
void setSetting(dc1394camera_t *cam, dc1394feature_t setting, float value) {
    ...
    if (value < min || value > max) {
        ...
        if (err != DC1394_SUCCESS) cerr << "Could not enable absolute control!" << endl;
        err = dc1394_feature_set_absolute_value(cam, setting, value);
        if (err != DC1394_SUCCESS) cerr << "Could not set absolute value!" << endl;
    }
}
...
void CameraIIDC::startCapture() {
    ...
    if (triggerMode == triggerModeHardware) {
        ...
        if (err != DC1394_SUCCESS) cerr << "libdc1394: Could not set external trigger on!";

        err = dc1394_external_trigger_set_source(cam, DC1394_TRIGGER_SOURCE_0);
        if (err != DC1394_SUCCESS) cerr << "libdc1394: Could not set external trigger source!";

        err = dc1394_external_trigger_set_mode(cam, DC1394_TRIGGER_MODE_14);
        if (err != DC1394_SUCCESS) cerr << "libdc1394: Could not set external trigger mode!";

        err = dc1394_external_trigger_set_polarity(cam, DC1394_TRIGGER_ACTIVE_HIGH);
        if (err != DC1394_SUCCESS) cerr << "libdc1394: Could not set external trigger polarity!";

    } else if (triggerMode == triggerModeSoftware) {
        // Disable hardware trigger
        err = dc1394_external_trigger_set_power(cam, DC1394_OFF);
        if (err != DC1394_SUCCESS) cerr << "libdc1394: Could not set external trigger off!";
    }
...
void CameraIIDC::flushBuffer() {
    ...
    while (!bufferEmpty) {
        if (dc1394_capture_dequeue(cam, DC1394_CAPTURE_POLICY_POLL, &currentFrame) ==
            DC1394_SUCCESS) {
...
size_t CameraIIDC::getFrameSizeBytes() {
    if (!capturing) {
        cerr << "ERROR: Cannot get frame size before capturing. Call startCapture() before "
                "getFrameSizeBytes()."
             << endl;
        return 0;
    }
