...
void CorrRejectOrgBoundFast::recomputeTargetBoundary() {
    ...
    pcl::PointCloud<pcl::PointXYZRGBNormal>::ConstPtr target =
        boost::static_pointer_cast<
            pcl::registration::DataContainer<pcl::PointXYZRGBNormal, pcl::PointNormal> >(
            data_container_)
            ->getInputTarget();

    if (!target->isOrganized()) {
        PCL_ERROR(
            "[CorrRejectOrgBoundFast::getRemainingCorrespondences] The target cloud is not "
            "organized.\n");
        return;
    }
        ...
        for (int x = 0; x < (int)boundary->width; x++) {
            ...
            for (int x_d = -window_size_ / 2; x_d <= window_size_ / 2; ++x_d) {
                for (int y_d = -window_size_ / 2; y_d <= window_size_ / 2; ++y_d) {
                    if (x + x_d >= 0 && x + x_d < target->width && y + y_d >= 0 &&
                        y + y_d < target->height) {
                        if (!pcl_isfinite(target->at(x + x_d, y + y_d).z) ||
                            fabs(target->at(x, y).z - target->at(x + x_d, y + y_d).z) >
    ...
    //    std::cout << "CorrRejectOrgBoundFast recomputeTargetBoundary(): " << time.elapsed() <<
    //    "ms" << std::endl;

    // pcl::io::savePCDFileASCII("boundary.pcd", *boundary);
}

void CorrRejectOrgBoundFast::getRemainingCorrespondences(
    ...
    for (size_t c_i = 0; c_i < original_correspondences.size(); ++c_i) {
        ...
        if (boundary->points[target_idx].label == 0)
            remaining_correspondences[num_remaining_correspondences++] =
                original_correspondences[c_i];
    }
