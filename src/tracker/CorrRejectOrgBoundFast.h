...
class CorrRejectOrgBoundFast : public CorrespondenceRejector {
   ...
   public:
    ...
    CorrRejectOrgBoundFast() { rejection_name_ = "CorrRejectOrgBoundFast"; }
    ...
    inline void setInputSource(const typename pcl::PointCloud<PointT>::ConstPtr &cloud) {
        if (!data_container_) data_container_.reset(new DataContainer<PointT>);
