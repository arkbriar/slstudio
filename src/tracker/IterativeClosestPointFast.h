...
class IterativeClosestPointFast : public Registration<PointSource, PointTarget, Scalar> {
   public:
    ...
    typedef boost::shared_ptr<const IterativeClosestPointFast<PointSource, PointTarget, Scalar> >
        ConstPtr;
    ...
    IterativeClosestPointFast()
        ...
        convergence_criteria_.reset(new pcl::registration::DefaultConvergenceCriteria<Scalar>(
     ...
     * class. This allows to check the convergence state after the align() method as well as to
     * configure DefaultConvergenceCriteria's parameters not available through the ICP API before
     * the align() method is called. Please note that the align method sets max_iterations_,
     * euclidean_fitness_epsilon_ and transformation_epsilon_ and therefore overrides the default /
     * set values of the DefaultConvergenceCriteria instance. \param[out] Pointer to the
     * IterativeClosestPoint's DefaultConvergenceCriteria.
     */
    ...
    virtual void setInputTarget(const PointCloudTargetConstPtr &cloud) {
        ...
        target_has_normals_ = false;
        for (size_t i = 0; i < fields.size(); ++i) {
            if (fields[i].name == "normal_x" || fields[i].name == "normal_y" ||
                fields[i].name == "normal_z") {
   ...
   protected:
     ...
     */
    virtual void transformCloud(const PointCloudSource &input, PointCloudSource &output,
...
void IterativeClosestPointFast<PointSource, PointTarget, Scalar>::transformCloud(
    ...
    if (source_has_normals_) {
        ...
        for (size_t i = 0; i < input.size(); ++i) {
            ...
            if (!pcl_isfinite(pt[0]) || !pcl_isfinite(pt[1]) || !pcl_isfinite(pt[2])) continue;
            ...
            if (!pcl_isfinite(nt[0]) || !pcl_isfinite(nt[1]) || !pcl_isfinite(nt[2])) continue;
...
template <typename PointSource, typename PointTarget, typename Scalar>
void IterativeClosestPointFast<PointSource, PointTarget, Scalar>::computeTransformation(
    ...
    do {
        ...
        if (use_reciprocal_correspondence_)
            correspondence_estimation_->determineReciprocalCorrespondences(*correspondences_,
        ...
        if (cnt < min_number_correspondences_) {
            PCL_ERROR(
                "[pcl::%s::computeTransformation] Not enough correspondences found. Relax your "
