...
TrackerPCL::TrackerPCL() {
    ...
    boost::shared_ptr<pcl::tracking::DistanceCoherence<pcl::PointXYZRGB> > distance_coherence =
        boost::shared_ptr<pcl::tracking::DistanceCoherence<pcl::PointXYZRGB> >(
            new pcl::tracking::DistanceCoherence<pcl::PointXYZRGB>());
    // coherence->addPointCoherence (distance_coherence);

    boost::shared_ptr<pcl::search::Octree<pcl::PointXYZRGB> > search(
        new pcl::search::Octree<pcl::PointXYZRGB>(0.01));
...
void TrackerPCL::setReference(PointCloudConstPtr refPointCloud) {
    ...
    std::cout << "Reduced number of points in source from " << refPointCloud->points.size()
              << " to " << filteredPointCloud->points.size() << std::endl;
    ...
    pcl::transformPointCloud<pcl::PointXYZRGB>(*filteredPointCloud, *demeanPointCloud,
                                               trans.inverse());
...
void TrackerPCL::determineTransformation(PointCloudConstPtr pointCloud, Eigen::Affine3f &T,
