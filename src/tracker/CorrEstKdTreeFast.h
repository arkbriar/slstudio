...
class CorrEstKdTreeFast : public CorrespondenceEstimationBase<PointSource, PointTarget, Scalar> {
   public:
    ...
    CorrEstKdTreeFast() { corr_name_ = "CorrEstKdTreeFast"; }
    ~CorrEstKdTreeFast() {}
    ...
    void determineReciprocalCorrespondences(
        Correspondences &correspondences, double max_distance = std::numeric_limits<double>::max());
};

// Templated class methods have to be implemented in the header...

template <typename PointSource, typename PointTarget, typename Scalar>
void CorrEstKdTreeFast<PointSource, PointTarget, Scalar>::determineCorrespondences(
    ...
    if (isSamePointType<PointSource, PointTarget>()) {
        ...
        for (std::vector<int>::const_iterator idx = indices_->begin(); idx != indices_->end();
             ++idx) {
            if (isFinite(input_->points[*idx])) {
                tree_->nearestKSearch(input_->points[*idx], 1, index, dist_sqr);
                if (dist_sqr[0] > max_dist_sqr) continue;
        ...
        for (std::vector<int>::const_iterator idx = indices_->begin(); idx != indices_->end();
            ...
            if (isFinite(input_->points[*idx])) {
                ...
                pcl::for_each_type<FieldListTarget>(
                    pcl::NdConcatenateFunctor<PointSource, PointTarget>(input_->points[*idx], pt));
...
template <typename PointSource, typename PointTarget, typename Scalar>
void CorrEstKdTreeFast<PointSource, PointTarget, Scalar>::determineReciprocalCorrespondences(
    pcl::Correspondences &correspondences, double max_distance) {
    if (!initCompute()) return;
    ...
    if (isSamePointType<PointSource, PointTarget>()) {
        ...
        for (std::vector<int>::const_iterator idx = indices_->begin(); idx != indices_->end();
            ...
            tree_reciprocal_->nearestKSearch(target_->points[target_idx], 1, index_reciprocal,
                                             distance_reciprocal);
            if (distance_reciprocal[0] > max_dist_sqr || *idx != index_reciprocal[0]) continue;
        ...
        for (std::vector<int>::const_iterator idx = indices_->begin(); idx != indices_->end();
            ...
            pcl::for_each_type<FieldList>(
                pcl::NdConcatenateFunctor<PointSource, PointTarget>(input_->points[*idx], pt_src));
            ...
            pcl::for_each_type<FieldList>(pcl::NdConcatenateFunctor<PointTarget, PointSource>(
                target_->points[target_idx], pt_tgt));
