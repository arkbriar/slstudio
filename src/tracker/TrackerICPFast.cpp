...
TrackerICPFast::TrackerICPFast() { lastTransformation = Eigen::Matrix4f::Identity(); }
        ...
        for (unsigned int j = 0; j < refPointCloud->height; j++) {
            ...
            for (unsigned int x_d = -windowSize / 2; x_d <= windowSize / 2; ++x_d) {
                for (unsigned int y_d = -windowSize / 2; y_d <= windowSize / 2; ++y_d) {
                    if (i + x_d >= 0 && i + x_d < refPointCloud->width && j + y_d >= 0 &&
                        j + y_d < refPointCloud->height) {
                        bool nan = !pcl::isFinite(refPointCloud->at(i + x_d, j + y_d));
                        bool step = fabs(refPointCloud->at(i, j).z -
                                         refPointCloud->at(i + x_d, j + y_d).z) > depthThreshold;
                        if (nan || step) nanCount++;
                    }
                }
            }
            if (nanCount > boundaryThreshold) boundary.at<bool>(i, j) = true;
        }
    }
}

void TrackerICPFast::determineTransformation(PointCloudConstPtr pointCloud, Eigen::Matrix4f &T,
                                             bool &converged) {
