...
class CorrEstOrgProjFast : public CorrespondenceEstimationBase<PointSource, PointTarget, Scalar> {
   public:
    ...
    boost::shared_ptr<CorrespondenceEstimationBase<PointSource, PointTarget, Scalar> > clone()
        const {
        CorrEstOrgProjFast *clone = new CorrEstOrgProjFast();
        clone->setProjectionMatrix(this->projection_matrix_);
        return boost::shared_ptr<CorrespondenceEstimationBase<PointSource, PointTarget, Scalar> >(
            clone);
    }
    ~CorrEstOrgProjFast() {}
...
template <typename PointSource, typename PointTarget, typename Scalar>
void CorrEstOrgProjFast<PointSource, PointTarget, Scalar>::determineCorrespondences(
    ...
    for (std::vector<int>::const_iterator src_it = indices_->begin(); src_it != indices_->end();
         ++src_it) {
        if (isFinite(input_->points[*src_it])) {
            ...
            if (u >= 0 && u < static_cast<int>(target_->width) && v >= 0 &&
                ...
                if (!isFinite(pt_tgt)) continue;

                float depth_dist = fabs(uv[2] - pt_tgt.z);
                if (depth_dist > max_distance) continue;

                correspondences[c_index++] = pcl::Correspondence(*src_it, v * target_->width + u,
                                                                 static_cast<float>(depth_dist));
            }
        }
    }
