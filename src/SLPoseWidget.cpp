...
void SLPoseWidget::showPoseEstimate(Eigen::Affine3f T) {
    ...
    visualizer->setCameraPosition(posVector[0], posVector[1], posVector[2], lookAtVector[0],
                                  lookAtVector[1], lookAtVector[2], upVector[0], upVector[1],
                                  upVector[2]);

    //    // Workaround 2
    //    double dummy;
    //    if(!visualizer->getPointCloudRenderingProperties(pcl::visualization::PCL_VISUALIZER_POINT_SIZE,
    //    dummy, "pointCloudPose"))
    //        visualizer->addPointCloud(pointCloudPCL, *colorHandler, "pointCloudPose",
    //        viewPortPose);
