...
void SLTraceWidget::addMeasurements(std::vector<float> vals) {
    if (vals.size() != traces.size()) return;

    for (unsigned int i = 0; i < vals.size(); i++) this->addMeasurement(i, vals[i]);
}

void SLTraceWidget::addMeasurement(unsigned int id, float val) {
    if (id > traces.size()) return;

    traces[id].push_back(val);

    if (traces[id].size() > tSpan) traces[id].erase(traces[id].begin());
}
...
void SLTraceWidget::draw() {
    ...
    for (unsigned int i = 0; i < traces.size(); i++) {
        ...
        // Write label
        cv::putText(im, QString("%1 = %2").arg(names[i]).arg(traces[i].back()).toStdString(),
                    cv::Point(10, (i + 1) * 20), cv::FONT_HERSHEY_PLAIN, 1.0, color, 1.8);
    }
