...
void SLTrackerWorker::setup() {
    ...
    Eigen::Matrix3f Kc;
    Kc << calibration.Kc(0, 0), calibration.Kc(0, 1), calibration.Kc(0, 2), calibration.Kc(1, 0),
        calibration.Kc(1, 1), calibration.Kc(1, 2), calibration.Kc(2, 0), calibration.Kc(2, 1),
        calibration.Kc(2, 2);
    ...
    if (writeToDisk) {
        ...
        (*ofStream) << "#V1.1 SLStudio Tracking File" << std::endl
                    << "t_ms,Tx,Ty,Tz,Q0,Qx,Qy,Qz,RMS" << std::endl;
...
void SLTrackerWorker::trackPointCloud(PointCloudConstPtr pointCloud) {
    ...
    // Emit result
    if (converged) emit newPoseEstimate(T);
