...
SLCalibrationDialog::SLCalibrationDialog(SLStudio *parent)
    ...
    connect(calibrator, SIGNAL(newSequenceResult(cv::Mat, unsigned int, bool)), this,
            SLOT(onNewSequenceResult(cv::Mat, uint, bool)));
...
void SLCalibrationDialog::on_snapButton_clicked() {
    ...
    QListWidgetItem *item =
        new QListWidgetItem(QString("Sequence %1").arg(frameSeqs.size()), ui->listWidget);
