...
void phaseCorrelate(const cv::Mat &im1, const cv::Mat &im2, float &scale, float &angle,
                    cv::Point2f &shift) {
    ...
    //    cv::Point2f scaleRotation = phasecorrelation::phaseCorrelate(im1LogPolar, im2LogPolar,
    //    window);
    ...
    //    cv::Mat scaleRotationMatrix = cv::getRotationMatrix2D(cv::Point2f(im1.cols/2.0,
    //    im1.rows/2.0), angle, scale); cv::warpAffine(im1Float, im1ScaledRotated,
...
cv::Mat logPolar(const cv::Mat &image, float scale) {
    ...
    cvLogPolar(&imageIpl, &resultIpl, cv::Point2f(imageIpl.width / 2.0, imageIpl.height / 2.0),
               scale);
...
// Convention for distortion parameters:
// http://www.vision.caltech.edu/bouguetj/calib_doc/htmls/parameters.html
void initDistortMap(const cv::Matx33f cameraMatrix, const cv::Vec<float, 5> distCoeffs,
...
// Downsample a texture which was created in virtual column/row space for a diamond pixel array
// projector
    ...
    if (evt == cv::EVENT_LBUTTONDOWN) {
        if (im->type() == CV_8UC3) {
            printf("%d %d: %d, %d, %d\n", x, y, (int)(*im).at<cv::Vec3b>(y, x)[0],
                   (int)(*im).at<cv::Vec3b>(y, x)[1], (int)(*im).at<cv::Vec3b>(y, x)[2]);
...
cv::Mat histimage(cv::Mat histogram) {
    ...
    for (int i = 1; i < histogram.rows - 10; i++) {
        cv::line(histImage,
                 cv::Point(bin_w * (i - 1), histImage.rows - cvRound(histogram.at<float>(i - 1))),
