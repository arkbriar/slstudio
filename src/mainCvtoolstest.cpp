...
int main() {
    ...
    cv::Mat scaleRotationMatrix =
        cv::getRotationMatrix2D(cv::Point2f(im.cols / 2.0, im.rows / 2.0), 10.0, 1.0);
    ...
    cv::warpAffine(imTransformed, imTransformed, translationMatrix, imTransformed.size(),
                   CV_INTER_LINEAR, cv::BORDER_CONSTANT, 0.0);
