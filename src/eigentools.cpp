...
// Algorithm and notation according to Mili Shah, Comparing two sets ofcorresponding six degree of
// freedom data, CVIU 2011. DTU, 2013, Oline V. Olesen, Jakob Wilm
void fitSixDofData(const std::vector<Eigen::Affine3f> X, const std::vector<Eigen::Affine3f> X_mark,
                   Eigen::Affine3f &H) {
    // NOT DEBUGGED!
    int N = X.size();
    ...
    // SVD
    Eigen::JacobiSVD<Eigen::MatrixXf> svd(X_bar * X_mark_bar.transpose(),
                                          Eigen::ComputeThinU | Eigen::ComputeThinV);

    Eigen::Matrix3f D = Eigen::Matrix3f::Identity();
    if ((svd.matrixV() * svd.matrixU().transpose()).determinant() < 0) D(3, 3) = -1;
