...
void showError(std::string err) { std::cerr << "lc4500startup: " << err.c_str() << std::endl; }
...
ProjectorLC4500::ProjectorLC4500(unsigned int) : nPatterns(0), isRunning(false) {
    ...
    // int LCR_AddToPatLut(int TrigType, int PatNum,int BitDepth,int LEDSelect,bool InvertPat, bool
    // InsertBlack,bool BufSwap, bool trigOutPrev)
    ...
    if (LCR_SendPatLut() < 0) showError("Error Sending Pattern LUT");
    ...
    unsigned int status;
    if (LCR_ValidatePatLutData(&status) < 0) showError("Error validating LUT data");

    // Set trigger signal configuration
    LCR_SetTrigOutConfig(1, true, 0, 0);
}

void ProjectorLC4500::setPattern(unsigned int patternNumber, const unsigned char *tex,
...
void ProjectorLC4500::displayTexture(const unsigned char *tex, unsigned int texWidth,
