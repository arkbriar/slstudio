...
ErrorCode_t LCR_CMD_SetDisplayMode(LCR_DisplayMode_t Mode) {
    if (Mode > DISP_MODE_PTN_SEQ) return FAIL;
...
ErrorCode_t LCR_CMD_SetTestPattern(LCR_TestPattern_t TestPtn) {
    if (TestPtn > TEST_PTN_ANXI_CHECKER) return FAIL;
...
ErrorCode_t LCR_CMD_SetPatternSeqSetting(LCR_PatternSeqSetting_t const *Setting) {
    LCR_CMD_PKT_CommandInit(
        LCR_CMD_PKT_TYPE_WRITE,
        0x0480);  // The old cmd 0x0400 is kept for backward compatibility only use the new one
    LCR_CMD_PKT_PutInt((int)Setting->BitDepth, 1);
...
/* Displays next pattern in pattern sequence applicable only if Input Trigger Type = Command Trigger
 * (0x00) */
