...
ErrorCode_t LCR_CMD_PKT_ConnectToLCR(void) {
    ...
    if (LCR_PKT_Socket < 0) return FAIL;
...
int LCR_CMD_ReadData(uint8 *data, uint32 size) { return TCP_Receive(LCR_PKT_Socket, data, size); }

int LCR_CMD_WriteData(uint8 *data, uint32 size) { return TCP_Send(LCR_PKT_Socket, data, size); }
...
int LCR_CMD_PKT_PutData(uint8 *data, unsigned long int size) {
    ...
    while (size) {
        ...
        if (dataLength == MAX_PACKET_SIZE) {
            if (LCR_CMD_PKT_SendPacket(1)) return -1;
            dataLength = 0;
        }
...
int LCR_CMD_PKT_PutInt(uint32 value, unsigned int size) {
    ...
    if (LCR_CMD_PKT_PutData(data, size)) return -1;
...
int LCR_CMD_PKT_PutFile(char const *fileName) {
    ...
    while (!feof(fp)) {
        ...
        copySize = fread(packetData + dataLength, 1, copySize, fp);
        if (copySize <= 0) break;
...
int LCR_CMD_PKT_GetFile(char const *fileName, uint32 size) {
    ...
    if (remSize == 0) remSize = MAX_PACKET_SIZE;
...
int LCR_CMD_PKT_GetData(uint8 *data, unsigned long int size) {
    ...
    while (size) {
        ...
        if (copySize == 0) {
            if (LCR_CMD_PKT_ReceivePacket(0)) return -1;
...
uint32 LCR_CMD_PKT_GetInt(unsigned int size) {
    ...
    if (LCR_CMD_PKT_GetData(data, size)) return 0;
...
int LCR_CMD_PKT_ReceivePacket(BOOL firstPkt) {
    ...
    if (packetBuffer[0] != LCR_PacketType + 1) {
        ...
        switch (packetBuffer[0]) {
            ...
            case 0x04:
                printf(
                    "COMMAND PACKET ERROR: DM365 sent back command type as Write = [0x02] or Read "
                    "= [0x04] command!!!\n");
                break;
...
int LCR_CMD_PKT_SendPacket(BOOL more) {
    ...
    if (LCR_CMD_WriteData(packetBuffer, length)) return -1;

    if (LCR_CMD_PKT_ReceivePacket(1)) return -1;
...
int LCR_CMD_PKT_SendCommand(void) { return LCR_CMD_PKT_SendPacket(0); }
