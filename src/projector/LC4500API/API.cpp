 ...
 * API.cpp
 *
 * This module provides C callable APIs for each of the command supported by LightCrafter4500
 * platform and detailed in the programmer's guide.
 *
...
int LCR_Write() { return USB_Write(); }
...
int LCR_ContinueRead() { return USB_Read(); }
...
int LCR_SetLedEnables(bool SeqCtrl, bool Red, bool Green, bool Blue) {
    ...
    if (SeqCtrl) Enable |= BIT3;
    if (Red) Enable |= BIT0;
    if (Green) Enable |= BIT1;
    if (Blue) Enable |= BIT2;
...
int LCR_DownloadData(unsigned char *pByteArray, unsigned int dataLen) {
    ...
    sendSize = HID_MESSAGE_MAX_SIZE - sizeof(msg.head) - sizeof(msg.text.cmd) -
               2;  // The last -2 is to workaround a bug in bootloader.

    if (dataLen > sendSize) dataLen = sendSize;
...
void LCR_WaitForFlashReady() {
    ...
    } while ((BLstatus & STAT_BIT_FLASH_BUSY) ==
             STAT_BIT_FLASH_BUSY);  // Wait for flash busy flag to go off
}
...
int LCR_SetFPD_Mode_Field(unsigned int PixelMappingMode, bool SwapPolarity,
                          unsigned int FieldSignalSelect) {
    hidMessageStruct msg;

    msg.text.data[2] = PixelMappingMode << 6;
    msg.text.data[2] |= FieldSignalSelect & (BIT0 | BIT1 | BIT2);
    if (SwapPolarity) msg.text.data[2] |= BIT3;
    LCR_PrepWriteCmd(&msg, FPD_MODE);

    return LCR_SendMsg(&msg);
}

int LCR_GetFPD_Mode_Field(unsigned int *pPixelMappingMode, bool *pSwapPolarity,
...
int LCR_SetTPGColor(unsigned short redFG, unsigned short greenFG, unsigned short blueFG,
                    unsigned short redBG, unsigned short greenBG, unsigned short blueBG) {
    hidMessageStruct msg;
...
int LCR_GetTPGColor(unsigned short *pRedFG, unsigned short *pGreenFG, unsigned short *pBlueFG,
                    unsigned short *pRedBG, unsigned short *pGreenBG, unsigned short *pBlueBG) {
    hidMessageStruct msg;
...
int LCR_AddToPatLut(int TrigType, int PatNum, int BitDepth, int LEDSelect, bool InvertPat,
                    bool InsertBlack, bool BufSwap, bool trigOutPrev) {
    unsigned int lutWord = 0;

    lutWord = TrigType & 3;
    if (PatNum > 24) return -1;

    lutWord |= ((PatNum & 0x3F) << 2);
    if ((BitDepth > 8) || (BitDepth <= 0)) return -1;
    lutWord |= ((BitDepth & 0xF) << 8);
    if (LEDSelect > 7) return -1;
    lutWord |= ((LEDSelect & 0x7) << 12);
    if (InvertPat) lutWord |= BIT16;
    if (InsertBlack) lutWord |= BIT17;
    if (BufSwap) lutWord |= BIT18;
    if (trigOutPrev) lutWord |= BIT19;
...
int LCR_GetPatLutItem(int index, int *pTrigType, int *pPatNum, int *pBitDepth, int *pLEDSelect,
                      bool *pInvertPat, bool *pInsertBlack, bool *pBufSwap, bool *pTrigOutPrev) {
...
int LCR_SendPatLut(void) {
    ...
    if (LCR_OpenMailbox(2) < 0) return -1;
...
int LCR_SendSplashLut(unsigned char *lutEntries, unsigned int numEntries) {
    ...
    if (numEntries < 1 || numEntries > 64) return -1;
...
int LCR_GetPatLut(int numEntries) {
    ...
    if (numEntries > 128) return -1;

    if (LCR_OpenMailbox(2) < 0) return -1;

    if (LCR_MailboxSetAddr(0) < 0) return -1;
    ...
    if (LCR_CloseMailbox() < 0) return -1;
...
int LCR_GetSplashLut(unsigned char *pLut, int numEntries) {
    ...
    if (LCR_OpenMailbox(1) < 0) return -1;
...
int LCR_SetPatternConfig(unsigned int numLutEntries, bool repeat, unsigned int numPatsForTrigOut2,
                         unsigned int numSplash) {
    hidMessageStruct msg;

    msg.text.data[2] =
        numLutEntries - 1; /* -1 because the firmware command takes 0-based indices (0 means 1) */
    msg.text.data[3] = repeat;
    msg.text.data[4] = numPatsForTrigOut2 -
                       1; /* -1 because the firmware command takes 0-based indices (0 means 1) */
    msg.text.data[5] =
        numSplash - 1; /* -1 because the firmware command takes 0-based indices (0 means 1) */
    LCR_PrepWriteCmd(&msg, PAT_CONFIG);

    return LCR_SendMsg(&msg);
}

int LCR_GetPatternConfig(unsigned int *pNumLutEntries, bool *pRepeat,
                         unsigned int *pNumPatsForTrigOut2, unsigned int *pNumSplash) {
    hidMessageStruct msg;
    ...
    if (LCR_Read() > 0) {
        memcpy(&msg, InputBuffer, 65);
        *pNumLutEntries =
            msg.text.data[0] + 1; /* +1 because the firmware gives 0-based indices (0 means 1) */
        *pRepeat = (msg.text.data[1] != 0);
        *pNumPatsForTrigOut2 =
            msg.text.data[2] + 1; /* +1 because the firmware gives 0-based indices (0 means 1) */
        *pNumSplash =
            msg.text.data[3] + 1; /* +1 because the firmware gives 0-based indices (0 means 1) */
        return 0;
    }
    return -1;
}
...
int LCR_GetExposure_FramePeriod(unsigned int *pExposure, unsigned int *pFramePeriod) {
    ...
    if (LCR_Read() > 0) {
        memcpy(&msg, InputBuffer, 65);
        *pExposure = msg.text.data[0] | msg.text.data[1] << 8 | msg.text.data[2] << 16 |
                     msg.text.data[3] << 24;
        *pFramePeriod = msg.text.data[4] | msg.text.data[5] << 8 | msg.text.data[6] << 16 |
                        msg.text.data[7] << 24;
        return 0;
    }
    return -1;
...
int LCR_SetTrigOutConfig(unsigned int trigOutNum, bool invert, unsigned int rising,
...
int LCR_GetTrigOutConfig(unsigned int trigOutNum, bool *pInvert, unsigned int *pRising,
                         unsigned int *pFalling) {
    hidMessageStruct msg;
...
int LCR_GetTrigIn1Delay(unsigned int *pDelay) {
    ...
    if (LCR_Read() > 0) {
        memcpy(&msg, InputBuffer, 65);
        *pDelay = msg.text.data[0] | msg.text.data[1] << 8 | msg.text.data[2] << 16 |
                  msg.text.data[3] << 24;
        return 0;
    }
    return -1;
...
int LCR_GetPWMConfig(unsigned int channel, unsigned int *pPulsePeriod, unsigned int *pDutyCycle) {
    ...
    if (LCR_Read() > 0) {
        memcpy(&msg, InputBuffer, 65);
        *pPulsePeriod = msg.text.data[1] | msg.text.data[2] << 8 | msg.text.data[3] << 16 |
                        msg.text.data[4] << 24;
...
int LCR_GetPWMCaptureConfig(unsigned int channel, bool *pEnabled, unsigned int *pSampleRate) {
    ...
    if (LCR_Read() > 0) {
        ...
        *pSampleRate = msg.text.data[1] | msg.text.data[2] << 8 | msg.text.data[3] << 16 |
                       msg.text.data[4] << 24;
...
int LCR_SetGPIOConfig(unsigned int pinNum, bool enAltFunc, bool altFunc1, bool dirOutput,
                      bool outTypeOpenDrain, bool pinState) {
    hidMessageStruct msg;
    unsigned char value = 0;

    if (enAltFunc) value |= BIT7;
    if (altFunc1) value |= BIT6;
    if (dirOutput) value |= BIT5;
    if (outTypeOpenDrain) value |= BIT4;
    if (pinState) value |= BIT3;
...
int LCR_GetGPIOConfig(unsigned int pinNum, bool *pEnAltFunc, bool *pAltFunc1, bool *pDirOutput,
...
int LCR_GetGeneralPurposeClockOutFreq(unsigned int clkId, bool *pEnabled,
...
int LCR_MemRead(unsigned int addr, unsigned int *readWord) {
    ...
    if (LCR_Read() > 0) {
        memcpy(&msg, InputBuffer, 65);
        *readWord = msg.text.data[0] | msg.text.data[1] << 8 | msg.text.data[2] << 16 |
                    msg.text.data[3] << 24;
        //*readWord = msg.text.data[3] | msg.text.data[2] << 8 | msg.text.data[1] << 16 |
        //msg.text.data[0] << 24; //MSB first
        return 0;
    }
    return -1;
}
...
int LCR_ReadSplashLoadTiming(unsigned int *pTimingData) {
    ...
    if (LCR_Read() > 0) {
        memcpy(&msg, InputBuffer, 65);
        for (i = 0; i < 12; i++) {
            *(pTimingData + i) = (msg.text.data[4 * i + 0] | msg.text.data[4 * i + 1] << 8 |
                                  msg.text.data[4 * i + 2] << 16 | msg.text.data[4 * i + 3] << 24);
        }
        return 0;
    }
    return -1;
