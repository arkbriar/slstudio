...
std::vector<ScreenInfo> OpenGLContext::GetScreenInfo() {
    ...
    if (!glfwInit()) std::cerr << "Could not initialize GLFW!" << std::endl;
...
OpenGLContext::OpenGLContext(uint _screenNum)
    : screenNum(_screenNum), screenResX(0), screenResY(0) {
    ...
    contextInfo->window =
        glfwCreateWindow(screenResX, screenResY, "GLFW OpenGL Context", contextInfo->monitor, NULL);
...
void OpenGLContext::setGamma(float gamma) { glfwSetGamma(contextInfo->monitor, gamma); }

void OpenGLContext::makeContextCurrent() { glfwMakeContextCurrent(contextInfo->window); }
