...
ErrorCode_t LCR_CMD_DisplayPattern(LCR_PatternCount_t PtnNumber) {
    ...
    if (LCR_CMD_PKT_SendCommand()) return FAIL;
...
void ProjectorLC3000::setPattern(unsigned int patternNumber, const unsigned char *tex,
                                 unsigned int texWidth, unsigned int texHeight) {
    // Set pattern count
    unsigned int nPatterns =
        std::max((unsigned int)patternSeqSettings.NumPatterns, patternNumber + 1);
...
void ProjectorLC3000::displayPattern(unsigned int patternNumber) {
    ...
    if (this->ptn_seq_mode == false) {
        ...
        HandleResult(res, "LCR_CMD_SetDisplayMode()") this->ptn_seq_mode = true;
    }
...
void ProjectorLC3000::displayTexture(const unsigned char *tex, unsigned int texWidth,
...
void ProjectorLC3000::displayWhite() {
    ...
    HandleResult(res, "LCR_CMD_DisplayStaticColor()") this->ptn_seq_mode = false;
}
