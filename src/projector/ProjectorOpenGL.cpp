...
ProjectorOpenGL::ProjectorOpenGL(unsigned int _screenNum) {
    ...
    GLenum err = glewInit();
    if (err != GLEW_OK) std::cerr << "ProjectorOpenGL: Could not initialize GLEW!" << std::endl;
...
void ProjectorOpenGL::setPattern(unsigned int patternNumber, const unsigned char *tex,
    ...
    if (patternNumber + 1 == frameBuffers.size() + 1) {
        ...
        std::cerr << "ProjectorOpenGL: cannot set pattern " << patternNumber << " before setting "
                  << frameBuffers.size() << " -- " << patternNumber - 1 << std::endl;
        return;
    }
...
void ProjectorOpenGL::displayPattern(unsigned int patternNumber) {
    if (patternNumber + 1 > frameBuffers.size()) {
        std::cerr << "ProjectorOpenGL: cannot display pattern " << patternNumber
    ...
    glBlitFramebuffer(0, 0, context->getScreenResX(), context->getScreenResY(), 0, 0,
                      context->getScreenResX(), context->getScreenResY(), GL_COLOR_BUFFER_BIT,
                      GL_NEAREST);
...
void ProjectorOpenGL::displayTexture(const unsigned char *tex, unsigned int texWidth,
