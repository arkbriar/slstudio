...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (!strcmp("new", cmd)) {
        ...
        if (nrhs < 2) mexErrMsgTxt("New: Expected screen number argument.");
    ...
    if (!strcmp("displayTexture", cmd)) {
        // Check parameters
        if (nlhs < 0 || nrhs < 5) mexErrMsgTxt("DisplayTexture: Unexpected arguments.");
        // Call the method
    ...
    if (!strcmp("displayWhite", cmd)) {
        // Check parameters
        if (nlhs < 0 || nrhs < 1) mexErrMsgTxt("Test: Unexpected arguments.");
        // Call the method
