...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (!strcmp("GetInterfaceCameraList", cmd)) {
        ...
        for (unsigned int i = 0; i < interfaceCameraList.size(); i++) {
            for (unsigned int j = 0; j < interfaceCameraList[i].size(); j++) {
                ...
                mxSetFieldByNumber(plhs[0], iCamera, 2,
                                   mxCreateString(interfaceCameraList[i][j].vendor.c_str()));
                mxSetFieldByNumber(plhs[0], iCamera, 3,
                                   mxCreateString(interfaceCameraList[i][j].model.c_str()));
                iCamera++;
            }
        }
        return;
    ...
    if (!strcmp("NewCamera", cmd)) {
        // Check parameters
        if (nlhs != 1) mexErrMsgTxt("NewCamera: One output expected.");
        if (nrhs < 3)
        ...
        plhs[0] =
            convertPtr2Mat<Camera>(Camera::NewCamera(interfaceNum, camNum, triggerModeSoftware));
        return;
    }
    ...
    if (!strcmp("delete", cmd)) {
        ...
        if (nlhs != 0 || nrhs != 2) mexWarnMsgTxt("Delete: Unexpected arguments ignored.");
        return;
    }
    ...
    if (!strcmp("startCapture", cmd)) {
        // Check parameters
        if (nrhs > 2) mexErrMsgTxt("startCapture: Unexpected arguments.");
        // Call the method
    ...
    if (!strcmp("stopCapture", cmd)) {
        // Check parameters
        if (nrhs > 2) mexErrMsgTxt("stopCapture: Unexpected arguments.");
        // Call the method
    ...
    if (!strcmp("getFrame", cmd)) {
        // Check parameters
        if (nlhs < 1 || nrhs > 2) mexErrMsgTxt("getFrame: Unexpected arguments.");
        // Call the method
